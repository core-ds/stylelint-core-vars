...
const VARS_AVAILABLE = isCoreComponentsMonorepo() || isVarsPackagedInstalled();
...
function loadVars(file) {
    ...
    if (isCoreComponentsMonorepo() || isVarsPackagedInstalled()) {
        ...
        root.walkDecls((decl) => {
            (result[normalize(decl.value)] ||= []).push(decl.prop);
        });
    } else {
 ...
 * @param {string} substr
 */
function findSubstring(str, substr, separator = ' ') {
    if (str === substr || str.startsWith(`${substr}${separator}`)) {
        return 0;
    } else {
        const value = `${separator}${substr}`;
        if (str.endsWith(value)) {
            return str.length - value.length + separator.length;
        } else {
            const index = str.indexOf(`${separator}${substr}${separator}`);

            return index === -1 ? index : index + separator.length;
        }
    }
}
